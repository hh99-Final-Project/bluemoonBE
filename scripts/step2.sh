...
sudo docker run -it --name ${TARGET_PORT} -d -p ${TARGET_PORT}:8080 -e active=${TARGET_PORT} -e JASYPT_ENCRYPTOR_PASSWORD=${password} -e JASYPT_ENCRYPTOR_REPEAT=${repeat} lion8548/spring-cicd:latest
#echo "sudo docker run -it --name ${TARGET_PORT} -d -p ${TARGET_PORT}:8080 -e active=${TARGET_PORT} lion8548/spring-cicd:latest"
echo "sudo docker run -it --name ${TARGET_PORT} -d -p ${TARGET_PORT}:8080 -e active=${TARGET_PORT} -e JASYPT_ENCRYPTOR_PASSWORD=${password} -e JASYPT_ENCRYPTOR_REPEAT=${repeat} lion8548/spring-cicd:latest"
echo 'good!!!!!!!!!!!!!!!!!'
